...
const accountCreation = async (f, password) => {
  ...
  it('copy icon is displayed and clickable 1', async function () {
    const field = await f.waitUntilShowUp(screens.main.iconCopy)
    await field.click()
    assert.notEqual(field, false, 'copy icon doesn\'t present')
  })

  it("Account's address is displayed and has length 20 symbols 1", async function () {
  ...
  it('Check clipboard buffer 1', async function () {
    const text = clipboardy.readSync()
    assert.equal(text.length, 42, "address account wasn't copied to clipboard")
  })

  it('open  \'Account name\' change dialog', async function () {
  ...
  it('fill out new account\'s name', async function () {
    const field = await f.waitUntilShowUp(screens.main.fieldAccountName)
    await field.clear()
    await field.sendKeys(newAccountName)
  })

  it('dialog \'Account name\' is disappeared if click button \'Save\'', async function () {
  ...
  it("Account's address is displayed and has length 20 symbols 2", async function () {
    const field = await f.waitUntilShowUp(screens.main.address)
    createdAccounts.push((await field.getText()).toUpperCase())
  ...
  it('accepts account password after lock', async function () {
  ...
  it('shows QR code option', async function () {
    const menu = await f.waitUntilShowUp(menus.dot.menu)
    await menu.click()
  ...
  it('checks QR code address is the same as account details address', async function () {
  ...
  it('copy icon is displayed and clickable 2', async function () {
    const field = await f.waitUntilShowUp(screens.QRcode.iconCopy)
    await field.click()
    assert.notEqual(field, false, 'copy icon doesn\'t present')
  })

  it('Check clipboard buffer 2', async function () {
    const text = clipboardy.readSync()
    assert.equal(text.length, 42, "address account wasn't copied to clipboard")
  })

  it('close QR code screen by clicking button arrow', async function () {
  ...
  it('close \'Info\' screen by clicking button arrow', async function () {
