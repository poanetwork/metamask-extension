...
const changePassword = async (f, password, newPassword) => {
  ...
  describe('Check screen "Settings" -> "Change password" ', function () {

    it('checks if current network name (localhost) is correct', async function () {
    ...
    it('error should not be displayed', async function () {
      const error = await f.waitUntilShowUp(screens.settings.error, 10)
      assert.equal(error, false, 'improper error is displayed')
    })

    it('checks if "Change password" button is present and enabled', async function () {
      const menu = await f.waitUntilShowUp(menus.sandwich.menu, 300)
      await menu.click()
      const settings = await f.waitUntilShowUp(menus.sandwich.settings)
      await settings.click()
    ...
    it('screen has correct title', async function () {
    ...
    it('screen contains correct label', async function () {
    ...
    it('clicking the button "No" bring back to "Setting" screen ', async function () {
  ...
  describe('Validation of errors ', function () {
    ...
    it('error if new password shorter than 8 digits', async function () {
    ...
    it('error if new password  doesn\'t match confirmation', async function () {
    ...
    it('error if new password match old password', async function () {
      await f.clearField(fieldNewPassword)
      await f.clearField(fieldConfirmNewPassword)
    ...
    it('error if old password incorrect', async function () {
    ...
    it('no errors if old, new, confirm new passwords are correct; user can change password', async function () {
      await f.clearField(fieldNewPassword)
      await f.clearField(fieldOldPassword)
      await f.clearField(fieldConfirmNewPassword)
  ...
  describe('Check if new password is accepted', function () {

    it('user can log out', async function () {
    ...
    it('can\'t login with old password', async function () {
      const field = await f.waitUntilShowUp(screens.lock.fieldPassword)
    ...
    it('accepts new password after lock', async function () {
      const field = await f.waitUntilShowUp(screens.lock.fieldPassword)
